...
function Contents(props: Props) {
  ...
  useMotionValueEvent(scrollY, "change", (y) => {
    ...
    if (y >= ranges[ranges.length - 1]) {
      setActiveSection(headings[headings.length - 1]);
      return;
    }
  ...
  return (
    <div className="flex flex-col">
      <span
        className={`${titleVisible ? "visible text-lg opacity-100" : "text-[0px] opacity-0"} font-medium truncate transition-all`}
        aria-hidden={!titleVisible}
      >
        {props.title}
      </span>
      <span className="mb-1 font-medium">Table of Contents</span>
      {props.ast.map((ev, idx) => (
        <ContentsEntry
          key={`${props.title}-${idx}-${ev.depth}`}
          href={ev.href}
          value={ev.value}
          depth={ev.depth}
          active={activeSection === ev.href}
        />
      ))}
    </div>
  );
